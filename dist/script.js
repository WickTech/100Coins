...
(function (CoinGeckoApi) {
    CoinGeckoApi["AllCoins"] = "coins/markets?vs_currency=inr&page=1&per_page=100&sparkline=false";
...
const CryptoUtility = {
    ...
    map: (data) => {
        return {
            ...
            id: data.id,
            image: data.image,
            marketCap: CryptoUtility.formatINR(data.market_cap),
            name: data.name,
            price: CryptoUtility.formatINR(data.current_price),
            rank: data.market_cap_rank,
            supply: data.circulating_supply.toLocaleString(),
            symbol: data.symbol,
            volume: CryptoUtility.formatINR(data.total_volume)
        };
    },
    mapAll: (data) => {
...
const ChartUtility = {
    ...
    getUrl: (id) => {
        return `${CoinGeckoApi.Base}/coins/${id}/market_chart?vs_currency=inr&days=1`;
    },
    mapPoints: (data) => {
